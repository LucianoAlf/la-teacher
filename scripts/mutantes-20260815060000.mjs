...
const MIGRATION = 'supabase/migrations/20260815060000_o_orfao_reclamado_volta_pro_inicio.sql'
const TEST = 'supabase/migrations/20260815060000_o_orfao_reclamado_volta_pro_inicio.test.sql'
const IMAGE = process.env.MUTANTE_20260815060000_POSTGRES_IMAGE ?? 'postgres:17-alpine'
...
const mutantes = [
  {
    // O mutante desta migration: o orfao e redespachado no estado em que
    // morreu. Producao provou (14:30 de 15/08) que a edge responde
    // {"status":"ignorado"} e a tentativa e queimada a toa.
    nome: 'redespacha o orfao no estado morto (a edge ignora)',
    ancora: "then 'pendente'",
    substituicao: "then status",
  },
  {
...
function extrairTesteDocker(texto) {
  const inicio = texto.indexOf('20260815060000-DOCKER-DML-TESTS-INICIO')
  const fim = texto.indexOf('20260815060000-DOCKER-DML-TESTS-FIM')
  if (inicio === -1 || fim === -1) {
    throw new Error('bloco DOCKER-DML-TESTS nao encontrado no teste')
  }
  return texto.slice(inicio + '20260815060000-DOCKER-DML-TESTS-INICIO'.length, fim)
}
...
const container = `la-teacher-20260815060000-mutantes-${process.pid}-${Date.now()}`
let iniciado = false
let mortos = 0
let invalido = false
