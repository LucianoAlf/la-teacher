-- 047 — a experimental se distingue na agenda
--
-- Ela ja aparecia: 29 aulas experimentais futuras, todas com roster, entram na
-- agenda como qualquer outra. So que uma experimental NAO e como qualquer
-- outra — e uma pessoa que nunca pisou aqui, cuja familia esta decidindo se
-- fica. O professor precisa enxergar isso antes de entrar na sala, e precisa
-- de um caminho pra ficha (045) e pro registro (038).
--
-- Tres campos novos na sessao, nenhum campo alterado — contrato so cresce, e
-- o cliente antigo continua lendo o que lia.
--
--   experimental       deriva de aulas_emusys.categoria (nao de nome de curso:
--                      curso muda, categoria e do sistema)
--   vinculo_id         a chave de todo o ciclo. PODE SER NULO: 19 das 29 tem
--                      vinculo, o resto o reconciliador ainda nao casou. Nulo
--                      significa "ainda nao da pra registrar", que e diferente
--                      de "nao e experimental" — a tela precisa dos dois.
--   experimental_nome  o nome de quem vem. Na experimental o que importa e a
--                      pessoa, nao a turma.
--
-- Teste: 047-experimental-na-agenda.test.sql
-- Mutantes: scripts/mutantes-047.mjs
  ...
  return coalesce((
    ...
    select jsonb_agg(
      jsonb_build_object(
        ...
        -- Experimental na agenda: ela JA aparecia aqui (29 futuras, todas com
        -- roster), so nao se distinguia de nada. Sem estes dois campos o card
        -- nao tem como mudar de cara nem como abrir a ficha.
        --
        -- Deriva de aulas_emusys.categoria, nao de uma lista de nomes de
        -- curso: curso muda, categoria e do sistema.
        'experimental', (ae.categoria = 'experimental'),
        -- O vinculo e a chave de TODO o ciclo (035/038/045). Pode vir nulo:
        -- 19 das 29 experimentais futuras tem vinculo, o resto o reconciliador
        -- ainda nao casou. Nulo tem significado — "ainda nao da pra registrar"
        -- — e a tela precisa saber a diferenca entre isso e "nao e experimental".
        'vinculo_id', (
          select v.id from public.lead_experimental_aulas v
           where v.aula_local_id = ae.id and v.cancelado_em is null
           order by v.id desc limit 1),
        'experimental_nome', (
          select le.nome_aluno from public.lead_experimental_aulas v
