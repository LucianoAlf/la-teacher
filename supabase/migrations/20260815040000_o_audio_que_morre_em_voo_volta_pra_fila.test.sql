...
create temporary table pg_temp._fabio_20260815040000_res (
  caso text,
  ok boolean,
  detalhe text
) on commit drop;

create or replace function pg_temp.checar_20260815040000(
  p_caso text,
  p_ok boolean,
  p_detalhe text
) returns void
...
as $function$
begin
  insert into pg_temp._fabio_20260815040000_res(caso, ok, detalhe)
...
declare
  ...
  v_norm text;
begin
  perform pg_temp.checar_20260815040000(
...
select json_build_object(
  'falhas', (select count(*) from pg_temp._fabio_20260815040000_res where not ok),
  'detalhe', coalesce((
    select json_agg(json_build_object(
      ...
      from pg_temp._fabio_20260815040000_res
     where not ok
  ), '[]'::json)
) as resumo;

/* 20260815040000-DOCKER-DML-TESTS-INICIO
...
20260815040000-DOCKER-DML-TESTS-FIM */
