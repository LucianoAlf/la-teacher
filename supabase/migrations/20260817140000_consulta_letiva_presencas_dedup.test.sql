-- CONSERTO do dedup (substitui o teste de 20260817130000, que afirmava a
-- contagem CRUA da view — certa pra view, errada pro professor).
...
begin
  ...
  perform pg_temp.checar('Rodrigo: presentes 23 (deduplicado; cru seria 40)',
    (v_res->>'presentes')::int = 23, coalesce(v_res->>'presentes','<nulo>'));
  perform pg_temp.checar('Rodrigo: faltas 5 (deduplicado; cru seria 7)',
    jsonb_array_length(v_res->'faltas') = 5, (v_res->'faltas')::text);
  perform pg_temp.checar('Rodrigo: falta_provavel 4 (deduplicado; cru seria 6)',
    jsonb_array_length(v_res->'falta_provavel') = 4, (v_res->'falta_provavel')::text);
  perform pg_temp.checar('Rodrigo: nao_aplicavel 7', jsonb_array_length(v_res->'nao_aplicavel') = 7, (v_res->'nao_aplicavel')::text);

  -- A PROVA DO CONSERTO: nenhum aluno aparece duas vezes no mesmo dia na lista
  -- de faltas. Foi assim que o defeito apareceu (Gabriel e Lavynea repetidos).
  perform pg_temp.checar('Rodrigo: nenhum aluno repetido no mesmo dia nas faltas',
    (select count(*) from (
       select x->>'aluno' a, x->>'data' d
         from jsonb_array_elements(v_res->'faltas') x
        group by 1,2 having count(*) > 1) q) = 0,
    (v_res->'faltas')::text);

  -- A asercao que protege aluno real: os baldes sao DISJUNTOS.
  perform pg_temp.checar('faltas nao engole falta_provavel (5, nao 9)',
    jsonb_array_length(v_res->'faltas') <> 9, jsonb_array_length(v_res->'faltas')::text);
  perform pg_temp.checar('faltas nao engole nao_aplicavel (5, nao 12)',
    jsonb_array_length(v_res->'faltas') <> 12, jsonb_array_length(v_res->'faltas')::text);

  -- ANTES eu afirmava que os baldes somavam o total de pares (aluno, aula
  -- operacional). MEDIDO e falso: para o prof. 35 os baldes somam 39 e existem
  -- 30 pares distintos. A diferenca sao CONFLITOS reais — a mesma aula com
  -- linhas de semanticas diferentes (a Emily, em 11/08, tem falta_confirmada E
  -- aula_justificada). Esta RPC nao resolve conflito; entao o invariante certo
  -- nao e a soma, e sim: NENHUM balde repete o mesmo aluno no mesmo dia.
  perform pg_temp.checar('nenhum balde repete aluno no mesmo dia',
    (select coalesce(sum(dup), 0) from (
       select (select count(*) from (
                 select x->>'aluno' a, x->>'data' dd
                   from jsonb_array_elements(v_res->balde) x
                  group by 1,2 having count(*) > 1) q) as dup
         from unnest(array['faltas','falta_provavel','indeterminado','nao_aplicavel']) balde
     ) t) = 0,
    'baldes com repetido');
  ...
  perform pg_temp.checar('Valdo: 9 faltas (deduplicado; cru seria 11)',
    jsonb_array_length(v_res->'faltas') = 9, (v_res->'faltas')::text);
  perform pg_temp.checar('Valdo: 0 falta_provavel',
    jsonb_array_length(v_res->'falta_provavel') = 0, (v_res->'falta_provavel')::text);
  perform pg_temp.checar('Valdo: presentes 31 (deduplicado; cru seria 61)',
    (v_res->>'presentes')::int = 31, coalesce(v_res->>'presentes','<nulo>'));
...
/* PRESENCAS-DEDUP-DOCKER-DML-TESTS-INICIO
...
begin
  ...
  v_res := public.fabio_professor_presencas_periodo(36, date '2026-08-11', date '2026-08-15');
  -- 3 e nao 4: o aluno 1 tem aula GEMEA (101 e 102 -> mesma aula operacional).
  perform pg_temp.checar('docker: presentes 3 (aula gemea deduplicada; sem dedup daria 4)',
    (v_res->>'presentes')::int = 3, coalesce(v_res->>'presentes','<nulo>'));
  -- 2 e nao 3: o aluno 4 tambem tem aula gemea (107 e 108 -> mesma operacional).
  perform pg_temp.checar('docker: faltas 2 (aula gemea deduplicada; sem dedup daria 3)',
    jsonb_array_length(v_res->'faltas') = 2, (v_res->'faltas')::text);
...
PRESENCAS-DEDUP-DOCKER-DML-TESTS-FIM */
