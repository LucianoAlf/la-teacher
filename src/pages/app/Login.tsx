 ...
 * pontinhos halftone, marca d'água LA, glow no avatar do Fábio —
...
export default function LoginPage() {
  ...
  return (
    <div className="flex h-dvh justify-center overflow-hidden" style={{ background: 'var(--login-bg)' }}>
      <div
        className="relative flex h-dvh w-full max-w-[430px] flex-col overflow-y-auto"
        style={{ background: 'var(--login-bg)', color: 'var(--login-text)' }}
      >
        {/* ---- ZONA DE IDENTIDADE — a capa ---- */}
        <div className="relative overflow-hidden px-7 pb-9 pt-14 text-center">
          {/* pontinhos halftone (rosa da LA, 20%) */}
          <div
            aria-hidden="true"
            className="pointer-events-none absolute inset-0"
            style={{ backgroundImage: 'radial-gradient(var(--login-dots) 1px, transparent 1.2px)', backgroundSize: '14px 14px' }}
          />
          {/* marca d'água LA (monograma, bem fraca) */}
          <svg
            aria-hidden="true"
            viewBox="0 0 943.27 1072.1"
            preserveAspectRatio="xMidYMid meet"
            className="pointer-events-none absolute left-1/2 top-3 w-[300px] -translate-x-1/2 -rotate-6"
