...
/**
 * Capa da marca (LA Teacher · família LA) — tela dark FIXA, independe do tema:
 * atmosfera rosa da casa (pontinhos halftone + marca d'água LA grande + glow no
 * avatar do Fábio), com o TEAL do Fábio só no botão. Todos os valores vêm de tokens.
 *
 * Mora aqui, e não dentro do Login, porque é a capa de TODAS as telas pré-login
 * (entrar, recuperar senha, definir nova senha). O `children` é o rodapé: cada
 * tela põe o próprio formulário e a capa não sabe qual é.
 */
export function CapaLogin({ children }: { children: ReactNode }) {
  return (
    <div className="flex h-svh justify-center overflow-hidden" style={{ background: 'var(--login-bg)' }}>
      ...
      >
        ...
        {/* ---- RODAPÉ — o formulário de cada tela (colado embaixo, igual o Organizer) ---- */}
        <div className="relative z-10 px-7 pb-10">{children}</div>
      </div>
    </div>
  )
}

/** Classe e estilo dos campos da capa — compartilhados pelas telas pré-login. */
export const campoCapa =
  'rounded-md px-[14px] py-[12px] text-sm outline-none placeholder:text-text-muted'

export const campoCapaStyle = {
...
export function RotuloCapa({ children }: { children: ReactNode }) {
  return (
...
/** Faixa de mensagem — erro (vermelho) ou aviso neutro. */
export function FaixaCapa({ tom, children }: { tom: 'erro' | 'aviso'; children: ReactNode }) {
  const erro = tom === 'erro'
  return (
    <div
      className="flex items-start gap-2 rounded-md px-3 py-[10px] text-[13px] font-semibold"
      style={
        erro
          ? { background: 'var(--login-danger-soft)', color: 'var(--login-danger)' }
          : { background: 'var(--login-input-bg)', color: 'var(--login-text)' }
      }
    >
      <i
        className={`fa-solid ${erro ? 'fa-circle-exclamation' : 'fa-circle-check'} mt-[2px]`}
        aria-hidden="true"
      />
      <span className="font-semibold leading-[1.45]">{children}</span>
    </div>
  )
}
